...
## 5. ?????? (Native Window Protocol)

??? Windows 11 ??????????????????,???? Win32 API ???????

### 5.1 ????
*   **??**: pp.py ?? _apply_win32_fullscreen_style ?????? HWND(????),???? DWM ?? (Corner Preference)?
*   **??**: ???? UniversalDriver ??? API ?? set_fullscreen?
*   **????**: ?? TitleBar ???? window ? esize ?????????,????????/?????

### 5.2 ????
1.  **???? CSS ??**: ???????? 100vh/100vw ????,????????????????
2.  **Idempotency (???)**: ?? set_fullscreen ??????,?????????????????
3.  **TitleBar ??**: ?????(???????)?????? <TitleBar /> ?????????? (Draggable Region) ??????
