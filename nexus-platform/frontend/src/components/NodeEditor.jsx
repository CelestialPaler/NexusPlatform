...
const NodeEditorContent = () => {
  ...
  const onDrop = useCallback(
    (event) => {
      ...
      let nodeData = { label: 'Node', type: 'default' };

      // Try parsing the payload from Sidebar
      if (payloadStr) {
        try {
          nodeData = JSON.parse(payloadStr);
        } catch (e) {
          console.error("Failed to parse node payload", e);
        }
      } else {
        // Fallback legacy logic for strings
        nodeData = { label: type, type: type };

        if (type === 'string') nodeData = { label: 'String', value: '', outputs: [{ id: 'val', type: 'string' }] }; // Fixed Type Logic
        if (type === 'integer') nodeData = { label: 'Integer', value: 0, outputs: [{ id: 'val', type: 'integer' }] };
        if (type === 'boolean') nodeData = { label: 'Boolean', value: false, outputs: [{ id: 'val', type: 'boolean' }] };
        if (type === 'log') nodeData = { label: 'Log', type: 'log', inputs: [{ id: 'in', type: 'flow' }, { id: 'msg', type: 'string' }], outputs: [{ id: 'out', type: 'flow' }] };
        if (type === 'ping') nodeData = { label: 'Ping', type: 'action', inputs: [{ id: 'in', type: 'flow' }, { id: 'ip', type: 'string' }], outputs: [{ id: 'out', type: 'flow' }, { id: 'rtt', type: 'integer' }] };
        if (type === 'trigger') nodeData = { label: 'Start', type: 'trigger', outputs: [{ id: 'exec', type: 'flow' }] };
        if (type === 'condition') nodeData = { label: 'Branch', type: 'condition', inputs: [{ id: 'in', type: 'flow' }, { id: 'cond', type: 'boolean' }], outputs: [{ id: 'true', type: 'flow' }, { id: 'false', type: 'flow' }] };
      }

      const newNode = {
        id: getId(),
        type: 'blueprintNode', // Always use our custom component container
        position,
        data: nodeData,
      };
